import React, { useEffect, useState } from "react";
import { AiOutlineCalendar } from "react-icons/ai";
import { newsApi, strapiBaseURl } from "../../../api/axios";
import { useRouter } from "next/navigation";
import TableShimmer from "../../../components/TableShimmer";

const News = ({ symbol }) => {
  const router = useRouter();
  const [news, setNews] = useState([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    // for news we are not using redux!!!
    setLoading(true);
    newsApi
      .get(
        `/newsses?populate=*&sort[0][createdAt]=desc&filters[companies][symbol][$eq]=${symbol}`
      )
      .then((res) => {
        setLoading(false);
        setNews(res.data.data);
      })
      .catch((err) => {
        setLoading(false);
        setNews(err.data);
      });
  }, [symbol]);
  const parseData = (string) => {
    var regex = /src="([^"]*)"/;
    var matches = `${string}`?.match(regex);
    if (string) {
      let data = `${string}`.replaceAll('src="', `src=\"${strapiBaseURl}`);
      return data;
    }
  };

  const fetchNewsDetails = (title, newsId) => {
    const replacedTitle = title.replace(/[% ]/g, "-");
    router.push(`/news/${replacedTitle}?newsId=${newsId}`);
  };
  return (
    <div>
      <div className="lg:text-sm text-3xl">
        <span className="text-[#6B6B6B]">Show : </span>
        <span className="font-semibold">News </span>
      </div>
      <div className="lg:container lg:mx-auto container flex lg:flex-row flex-col  py-5  lg:mt-0 gap-10  mx-auto">
        {loading ? (
          <TableShimmer />
        ) : (
          <>
            <div className=" text-center  lg:px-5  w-full lg:bg-secondary rounded-[20px] py-5">
              {news?.length > 0 ? (
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="font-semibold text-4xl lg:text-[18px]">
                      News
                    </h3>
                  </div>
                  <div>
                    <button className="ease-in duration-300  text-4xl lg:text-[18px] bg-secondary    border-2 rounded-md lg:text-xs font-semibold border-black px-3 hover:bg-black hover:text-secondary  py-1">
                      View more
                    </button>
                  </div>
                </div>
              ) : (
                ""
              )}

              {news?.length > 0 ? (
                <>
                  {news?.slice(0, 6)?.map((item, id) => {
                    return (
                      <>
                        <div
                          onClick={() => {
                            fetchNewsDetails(item?.attributes?.title, item?.id);
                          }}
                          key={id}
                          className="flex lg:flex-row flex-col gap-6 lg:py-4 pt-10 cursor-pointer pr-[20px]"
                        >
                          <div className="w-[60%] mx-auto h-[60%] lg:w-[20%] lg:h-[100px] ">
                            <img
                              className="w-full h-full object-contain"
                              src={
                                item?.attributes?.thumbnail?.data === null
                                  ? "https://peridotnepal.xyz/company_logo/sarallagani.webp"
                                  : strapiBaseURl +
                                    item?.attributes?.thumbnail?.data[0]
                                      ?.attributes?.formats?.thumbnail?.url
                              }
                              alt="Thumbnail"
                            />
                          </div>
                          <div className="w-[100%]">
                            <div className="flex flex-col lg:flex-row">
                              <h5 className="lg:text-[18px] text-[44px] capitalize font-semibold">
                                {item?.attributes?.title}
                              </h5>
                            </div>
                            <div className="flex mt-3 lg:mt-0 lg:gap-48 gap-80  py-2 lg:py-0 items-center">
                              <div className="flex items-center gap-3">
                                <div className="text-gray-500 text-[28px] lg:text-[14px]">
                                  <AiOutlineCalendar />
                                </div>
                                <div className="flex">
                                  <div className="lg:text-xs text-[28px]  text-gray-500">
                                    {item?.attributes?.createdAt?.split("T")[0]}
                                  </div>
                                </div>
                              </div>
                              <div></div>
                            </div>
                            <div>
                              <p className="lg:text-[12px] text-[34px] py-3 text-gray-700 h-[68px] overflow-hidden text-left">
                                {item?.attributes?.short_description
                                  ? item?.attributes?.short_description
                                  : "Lorem ipsum dolor sit amet consectetur adipisicing elit. Animi error praesentium natus inventore perferendis consectetur quia deserunt, assumenda illum. Minus libero architecto quia modi voluptatum facilis, officia molestias nemo fugit?"}
                              </p>
                            </div>
                          </div>
                        </div>
                        <hr />
                      </>
                    );
                  })}
                </>
              ) : (
                <div className="flex flex-col justify-center">
                  <svg
                    className="w-30 h-20"
                    id="Layer_1"
                    data-name="Layer 1"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 108.67 122.88"
                  >
                    <defs>
                      <style
                        dangerouslySetInnerHTML={{
                          __html: ".cls-1{fill-rule:evenodd;}",
                        }}
                      />
                    </defs>
                    <title>no-data</title>
                    <path
                      className="cls-1"
                      d="M25.14,53.37a2.77,2.77,0,0,0,0,5.54H45.25a2.77,2.77,0,0,0,0-5.54Zm60.48-36.9,6.66,6.69-8,8.14,8,8.14L85.61,46.1l-8-8.09-8,8.1L63,39.43l8-8.14-8-8.15,6.67-6.65,8,8.08,8-8.1ZM77.77,0A30.91,30.91,0,0,1,91,58.82v57.69a6.38,6.38,0,0,1-6.37,6.37H6.37A6.38,6.38,0,0,1,0,116.51V22.4A6.38,6.38,0,0,1,6.37,16h44.3A30.89,30.89,0,0,1,77.77,0Zm7.78,60.81A30.92,30.92,0,0,1,48.32,21.52H6.37a.9.9,0,0,0-.63.26.92.92,0,0,0-.26.63V116.5a.89.89,0,0,0,.89.89H84.65a.9.9,0,0,0,.63-.26.92.92,0,0,0,.26-.63V60.81ZM25.14,92.22a2.74,2.74,0,0,0,0,5.48H63.61a2.74,2.74,0,1,0,0-5.48Zm0-19.41a2.74,2.74,0,0,0,0,5.48H63.61a2.74,2.74,0,0,0,0-5.48Z"
                    />
                  </svg>
                  <h3 className="font-medium text-xl lg:text-sm py-1 text-gray-500">
                    NO DATA FOUND !
                  </h3>
                </div>
              )}
            </div>
          </>
        )}
      </div>
    </div>
  );
};

export default News;
