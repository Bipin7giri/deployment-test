...
const NewsIpo = () => {
  ...
  const funNewsApi = () => {
    ...
    newsApi
      .get(
        `/newsses?pagination[page]=${pagination.current}&pagination[pageSize]=${pagination.pageSize}&filters[categories][name][$contains]=IPO&populate=*&sort[0][publishedAt]=desc`
      )
      .then((res) => {
        setLoading(false);
  ...
  const getAllNewsData = () => {
    ...
    newsApi
      .get(
        `/newsses?pagination[page]=1&pagination[pageSize]=20&populate=thumbnail&sort[0][publishedAt]=desc`
      )
      .then((res) => {
        setLoading(false);
        setAllNews(true);
